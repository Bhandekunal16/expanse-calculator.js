...
class file_service_module:
    ...
    def write_file(self, folderPath: str, file_path: str, content: str):
        folder_path = os.path.join(
            self.home_directory, f"BudgetMateReports/{folderPath}"
        )
    ...
    async def maintain_account(path: str, updated_data: dict):
        ...
        if not file_exists:
            ...
            async with aiofiles.open(path, mode="r", encoding="utf-8") as file:
                ...
                if data_array:
                    csv_data = {
                        "totalExpanse": updated_data["totalExpanse"]
                        + float(data_array[0]["totalExpanse"]),
                        "totalIncome": updated_data["totalIncome"]
                        + float(data_array[0]["totalIncome"]),
                        "totalTransactions": updated_data["totalTransactions"]
                        + float(data_array[0]["totalTransactions"]),
                    }
                    async with aiofiles.open(path, mode="w", encoding="utf-8") as file:
